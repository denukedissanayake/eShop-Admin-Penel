...
const Login = () => {
  ...
  const login = async () => {
    ...
    if (data.accesToken) {
      ...
      const { username, name, email, contactdetails, location, role, photo, id } = userDetails as any;
      setUser({
        username, name, email, contactdetails, location, role, photo, accesToken: data.accesToken, id
      })
      navigation("/");
    } else {
